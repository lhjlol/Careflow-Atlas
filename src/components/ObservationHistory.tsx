...
import { supportCategoryLabels, type Observation, type OutreachSnapshot } from "../domain/types";
...
export function ObservationHistory({ snapshot, subjectId, subjectType = "UNIT", emptyLabel = "這個位置尚未有記錄" }: ObservationHistoryProps) {
  ...
  return <div className="cf-history" aria-label="位置記錄歷史">{observations.map((observation, index) => {
    ...
    return <article className={`cf-event ${index === 0 ? "is-latest" : ""}`} key={observation.id}>
      <div className="cf-event__rail" aria-hidden="true"><span /></div>
      <div className="cf-event__content"><div className="cf-event__meta"><span>{index === 0 ? "最新記錄" : "較早記錄"}</span><time dateTime={observation.recordedAt}>{formatDate(observation.recordedAt)}</time></div>
