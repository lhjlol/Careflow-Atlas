import { CalendarClock, CheckCircle2, CircleAlert, History, UserRound } from "lucide-react";
...
function formatDate(value?: string) { if (!value) return "未定日期"; return new Intl.DateTimeFormat("zh-HK", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(value)); }

export function ObservationHistory({ snapshot, subjectId, subjectType = "UNIT", emptyLabel = "這個位置尚未有記錄" }: ObservationHistoryProps) {
  const observations = snapshot.observations.filter((item) => subjectType === "UNIT" ? item.unitId === subjectId : item.buildingId === subjectId && !item.unitId).sort((a, b) => Date.parse(b.recordedAt) - Date.parse(a.recordedAt));
  if (!observations.length) return <div className="cf-empty"><History size={18} /><p>{emptyLabel}</p></div>;
  return <div className="cf-history" aria-label="位置記錄歷史">{observations.map((observation, index) => {
    const visit = snapshot.visits.find((item) => item.id === observation.visitId);
    const resolvedBy = snapshot.observations.find((item) => item.resolvesObservationId === observation.id);
    return <article className="cf-event" key={observation.id}>
      <div className="cf-event__rail" aria-hidden="true"><span /></div>
      <div className="cf-event__content"><div className="cf-event__meta"><span>{index === 0 ? "最新" : "較早"}</span><time dateTime={observation.recordedAt}>{formatDate(observation.recordedAt)}</time></div>
        <h4>{observation.note || observation.evidence[0] || coverageLabels[observation.coverage]}</h4>
        <div className="cf-chips"><span className="cf-chip"><UserRound size={14} />{observation.workerName}</span><span className="cf-chip">{coverageLabels[observation.coverage]}</span>{observation.contactOutcome && <span className="cf-chip">{contactLabels[observation.contactOutcome]}</span>}{observation.assessment && <span className="cf-chip cf-chip--uncertain"><CircleAlert size={14} />{assessmentLabels[observation.assessment]}</span>}</div>
        {observation.evidence.length > 0 && <p className="cf-event__note">依據：{observation.evidence.join("；")}</p>}
        <p className="cf-event__recorded">{visit ? `探訪：${formatDate(visit.occurredAt)} · ` : ""}記錄於 {formatDate(observation.recordedAt)}</p>
        {observation.followUp && <div className="cf-followup"><CalendarClock size={16} /><span><strong>{resolvedBy || observation.followUp.status === "DONE" ? "已由後續記錄結束" : "待跟進"}</strong> · {observation.followUp.action}<small>限期：{formatDate(observation.followUp.dueDate)}{resolvedBy ? ` · 結束於 ${formatDate(resolvedBy.occurredAt)}` : ""}</small></span></div>}
