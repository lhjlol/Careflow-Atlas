import { ArrowLeft, Building2, Check, ClipboardPlus, DoorOpen, Grid2X2, List, MapPin, RotateCcw } from "lucide-react";
import { useMemo, useState } from "react";
import { getCoverageStatus, getCoverageSummary, getOpenFollowUps, type CoverageStatus, type OutreachSnapshot } from "../domain/types";
...
export function BuildingDetail(props: BuildingDetailProps) {
  ...
  return <aside className="cf-building-panel" aria-label={`${building.name} 詳情`}>
    ...
    <section className="cf-floor-section"><div className="cf-section-heading"><div><span className="cf-eyebrow">樓層導航</span><h3>選擇樓層</h3></div><span>{building.layoutDeclared ? "示範佈局" : "佈局未核實"}</span></div>{floors.length ? <div className="cf-floor-strip" role="group" aria-label="樓層選擇">{floors.map((floor) => { const revisit = getCoverageSummary(snapshot, building.id, floor.id).followUps > 0; return <button key={floor.id} aria-pressed={activeFloorId === floor.id} className={`${activeFloorId === floor.id ? "is-active" : ""} ${revisit ? "has-followup" : ""}`} onClick={() => onSelectFloor(floor.id)}>{floor.label}{revisit && <span title="有待跟進"><RotateCcw size={12} />復訪</span>}</button>; })}</div> : <p className="cf-layout-note">尚未有已聲明的樓層或單位佈局，仍可記錄大廈層面的到訪。</p>}</section>
    {activeFloorId ? <section className="cf-unit-section" key={activeFloorId}><div className="cf-section-heading"><div><span className="cf-eyebrow">單位狀態</span><h3>{floors.find((item) => item.id === activeFloorId)?.label}<span className="cf-section-count">{units.length} 個單位</span></h3></div><div className="cf-view-toggle" data-view={view} aria-label="切換單位顯示"><button aria-pressed={view === "grid"} className={view === "grid" ? "is-active" : ""} onClick={() => setView("grid")} aria-label="方格顯示"><Grid2X2 /></button><button aria-pressed={view === "list"} className={view === "list" ? "is-active" : ""} onClick={() => setView("list")} aria-label="列表顯示"><List /></button></div></div><div className={`cf-units cf-units--${view}`}>{units.map((unit) => { const coverage = getCoverageStatus(snapshot, building.id, unit.id); const revisit = openFollowUps.some(item => item.unitId === unit.id); return <button key={unit.id} aria-pressed={selectedUnitId === unit.id} className={`${selectedUnitId === unit.id ? "is-active" : ""} cf-status-${coverage.toLowerCase()}`} onClick={() => onSelectUnit(unit.id)}><span><DoorOpen size={17} />{unit.label}{selectedUnitId === unit.id && <Check className="cf-unit-check" size={15} />}</span><small>{labels[coverage]}</small>{revisit && <em className="cf-unit-followup"><RotateCcw size={12} />待跟進</em>}</button>; })}</div></section> : floors.length > 0 && <p className="cf-selection-prompt">請先選擇樓層，再查看各單位狀態。</p>}
    {selectedUnit && <section className="cf-selected-unit" key={selectedUnit.id}><div className="cf-section-heading"><div><span className="cf-eyebrow">位置記錄</span><h3>{selectedUnit.label} · 到訪歷史</h3></div></div><ObservationHistory snapshot={snapshot} subjectId={selectedUnit.id} /></section>}
    {!selectedUnit && <section className="cf-selected-unit"><div className="cf-section-heading"><div><span className="cf-eyebrow">大廈層面</span><h3>到訪歷史</h3></div></div><ObservationHistory snapshot={snapshot} subjectId={building.id} subjectType="BUILDING" /></section>}
    </div>
